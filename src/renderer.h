...
private:
    ...
    int    m_fbWidth  = 0;          // current FBO texture size; resize() skips redundant recreation
    int    m_fbHeight = 0;
    ...
    int    m_gaussSize     = 4;     // per-level blur radius (small — the pyramid gives the spread)
    int    m_bloomLevels   = 6;     // pyramid levels summed at runtime (clamped to 1..allocated)
    float  m_bloomStrength = 4.0f;  // composite gain on the bloom; tune to taste
};
